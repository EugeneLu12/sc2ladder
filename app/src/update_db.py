...
from app.models.player import Player, Rank, Region
from app.src.laddercrawler import LadderCrawler
from app.src.parse import (
    parse_ladder,
    parse_ladder_legacy,
...
def update_database_legacy(region, ladder_list):
    for ladder in ladder_list:
        ...
        Player.players.bulk_create(
            ladder, batch_size=settings.DB_BATCH_SIZE, ignore_conflicts=True
        )
        Player.players.bulk_update(
            ladder,
...
def update_database_public_ladders(ladder_infos):
    for ladder in ladder_infos:
        ...
        Ladder.objects.update_or_create(
            ladder_id=ladder.ladder_id,
            region=ladder.p.region,
            defaults={
                "realm": ladder.p.realm,
