...
class PlayerInfo:
    ...
    def __init__(self, region: int, realm: int, profile_id: int, username: str = None, clan_tag: str = None,
                 race: str = None, wins: int = None, losses: int = None, mmr: int = None, league: str = None):
        self.region = region
        self.realm = realm
    ...
    def __repr__(self):
        return f"<PlayerInfo {self.username} {self.profile_id} {self.race} {self.region}>"
...
class LadderInfo:
    ...
    def __str__(self):
        return f"<LadderInfo https://starcraft2.com/en-us/api/sc2/" \
...
class LadderCrawler:
    ...
    async def _get_ladder_data(self, region: int, realm: int = None,
                               profile_id: int = None, ladder_id: int = None) -> dict:
        ...
        try:
            if profile_id is None:
                ...
                resp = await self.api.get_ladder(region=region, realm=realm, profile_id=profile_id, ladder_id=ladder_id)
    ...
    async def get_ladder_data(self, ladder_about: LadderInfo) -> List[PlayerInfo]:
        raw_ladder = await self._get_ladder_data(ladder_about.p.region, ladder_about.p.realm,
                                                 ladder_about.p.profile_id, ladder_about.ladder_id)
        p_list = list()
    ...
    async def get_ladder_infos(self, player: PlayerInfo) -> List[LadderInfo]:
        raw_summary = await self._get_summary(player.region, player.realm, player.profile_id)
        la_list = list()
    ...
    async def get_sub_ladders(self, root_ladder: LadderInfo = None, league: str = None,
                              region: int = None) -> List[LadderInfo]:
        ...
        ladder_infos = await asyncio.gather(*[self.get_ladder_infos(p) for p in players])
        ...
        if league is not None:
            vladder_infos = [l_info for l_info in ladder_infos if l_info.league == league.lower()]
        else:
        ...
        res = await asyncio.gather(*[self.get_sub_ladders(l_info, league) for l_info in vladder_infos])
        return ladder_infos + [item for sublist in res for item in sublist]

    async def get_adjacent_ladders(self, region=None, league=None, realm=None, profile_id=None, ladder_id=None):
        ladder_info = None
        ...
        return await self.get_sub_ladders(root_ladder=ladder_info, league=league, region=region)
